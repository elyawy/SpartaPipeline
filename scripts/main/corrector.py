...
sim_config = SimConfig(conf_file=args.config, seq_lengths=seq_lengths_in_msa)
...
def init_correction(sim_config, tree_data, num_sims):
    ...
    for params in tqdm(sim_params_correction):
    ...
    return correction_list, correction_list_sum_stats, substitution_model
msas_list, sum_stats_list, model_params = init_correction(sim_config, tree_data, args.numsim)

def get_regressors(msa_list, sum_stats_list, subsitution_model, sim_config):
...
regressors, performance = get_regressors(msas_list, sum_stats_list, model_params, sim_config)
...
if MAIN_PATH is None:
    ...
    pickle.dump(regressors, open(path_joiner(f'regressors_{sim_config.length_distribution}'), 'wb'))
    pd.DataFrame(performance).to_csv(path_joiner(f'regression_performance_{sim_config.length_distribution}.csv'))


# def apply_correction(original_matrix, correction_process):

#     correction = [np.fromstring(cor, dtype=float, sep=',') for cor in correction_process.get_result().split("\n")]
#     correction = np.array(correction).T
#     corrected_stats = np.matmul(original_matrix, correction)
